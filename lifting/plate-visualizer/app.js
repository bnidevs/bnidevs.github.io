(() => {
  ...
  function solvePlates(target) {
    if (target <= 0)
      return { plates: [0, 0, 0, 0], exact: target === 0, total: 0 };
      ...
      for (let b = 0; b <= maxB; b++) {
        ...
        for (let c = 0; c <= maxC; c++) {
          ...
          for (const d of [dLow, dLow + 1]) {
            ...
            const count = a + b + c + d;
            let dominated = diff > bestDiff;
            if (!dominated && best && diff === bestDiff) {
              const bestCount = best.plates.reduce((s, v) => s + v, 0);
              if (count > bestCount) {
                dominated = true;
              } else if (count === bestCount) {
                // prefer more of the heaviest plates (lexicographic)
                const cur = [a, b, c, d];
                dominated = true;
                for (let k = 0; k < 4; k++) {
                  if (cur[k] > best.plates[k]) {
                    dominated = false;
                    break;
                  }
                  if (cur[k] < best.plates[k]) break;
                }
              }
            }
            if (!dominated) {
  ...
  function calcTotal(counts) {
    return (
      BAR_WEIGHT + 2 * counts.reduce((s, c, i) => s + c * PLATES[i].weight, 0)
    );
  }
  ...
  function maxForPlate(index) {
    const otherWeight = plateCounts.reduce(
      (s, c, i) => (i === index ? s : s + c * PLATES[i].weight),
      0,
    );
  ...
  const BAR_HALF = 320,
    COLLAR_W = 10,
    COLLAR_START = 36;
  const PLATE_ZONE = BAR_HALF - 20; // full side minus end margins
  ...
  function drawBar(counts) {
    ...
    $svg.appendChild(
      rect(CX - COLLAR_START - COLLAR_W, CY - 12, COLLAR_W, 24, 2, "#999"),
    );
    ...
    const ordered = [];
    PLATES.forEach((p, i) => {
      for (let j = 0; j < counts[i]; j++) ordered.push(p);
    });
    ...
    const endMargin = 10;
    const stackW = n * pw + (n - 1) * pg;

    // Right side: flush right, heaviest leftmost (nearest collar)
    const rStart = CX + BAR_HALF - endMargin - stackW;
    ordered.forEach((p, i) => {
      const x = rStart + i * (pw + pg);
      $svg.appendChild(
        rect(
          x,
          CY - p.height / 2,
    ...
    // Left side: flush left, heaviest rightmost (nearest collar) — draw reversed
    const lStart = CX - BAR_HALF + endMargin;
    const reversed = [...ordered].reverse();
    reversed.forEach((p, i) => {
      const x = lStart + i * (pw + pg);
      $svg.appendChild(
        rect(
          x,
          CY - p.height / 2,
  ...
  function buildPlateRows() {
    PLATES.forEach((plate, i) => {
      ...
      swatch.className =
        "plate-swatch" + (plate.color === "#1c1c1c" ? " black" : "");
  ...
  function solveWeight(val) {
    ...
    return {
      ...
      exact: false,
      requested: val,
      actual: BAR_WEIGHT + 2 * best.total,
    };
  }
    ...
    if (mode === "plates") {
      ...
      if (!$weightInput.value) {
        ...
        if (!result.exact) {
          ...
          $weightMsgs.innerHTML = `<div class="msg-box warn">Can't hit ${result.requested} lbs exactly. Closest: ${result.actual} lbs (${delta > 0 ? "+" : ""}${delta} lbs)</div>`;
        } else {
