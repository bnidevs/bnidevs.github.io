...
let deltask = (e, k=null) => {
    ...
    const item_txt = pk.parentNode.parentNode.getElementsByClassName("item_txt")[0].innerText;
    let col = pk;
...
let edittask = (e) => {
    const item_txt = e.target.parentNode.parentNode.getElementsByClassName("item_txt")[0].innerText;
    ...
    field.addEventListener("keydown", (e) => {
        if(e.enterKey){
            ...
            e.target.parentNode.replaceChild(HTMLgen(`<p class="item_txt">${field.value}</p>`), e.target);
        }
    });
    ...
    e.target.parentNode.parentNode.replaceChild(field, e.target.parentNode.parentNode.getElementsByClassName("item_txt")[0]);

    savetasks();
}

let movetask = (editbtn, currcol, dir) => {
    const item_txt = editbtn.parentNode.parentNode.getElementsByClassName("item_txt")[0].innerText;
    if((currcol === "todo" && dir === LEFT) || (currcol === "done" && dir === RIGHT)){
        return;
    }
...
let addtask = (whichcol, tasktext="", fromsave=false) => {
    ...
    latest.getElementsByClassName("del_btn")[0].addEventListener("click", deltask);
    latest.getElementsByClassName("edit_btn")[0].addEventListener("click", edittask);

    latest.getElementsByClassName("left")[0].addEventListener("click", (e) => movetask(e.target, whichcol, LEFT));
    latest.getElementsByClassName("right")[0].addEventListener("click", (e) => movetask(e.target, whichcol, RIGHT));
...
let savetasks = () => {
    ...
    Object.keys(categories).forEach((k) => {
        document.getElementById(`${k}_add`).addEventListener("click", () => addtask(k, ""));
    });
}
