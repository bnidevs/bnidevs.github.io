...
fetch("https://gist.githubusercontent.com/cfreshman/a03ef2cba789d8cf00c08f767e0fad7b/raw/5d752e5f0702da315298a6bb5a771586d6ff445c/wordle-answers-alphabetical.txt")
	.then(resp => resp.text())
	.then(data => data.split("\n"))
	.then(data => {accepted_words = data});
	...
	for(const [k,v] of Object.entries(info)){
		if(!v.includes("-")){
			for(let i = 0; i < v.length; i++){
				if(v[i] >= 0 && w[v[i]] != k){
					console.log("eliminated: " + w + " -- " + w[i] + " is in place of " + k);
					return false;
				}
