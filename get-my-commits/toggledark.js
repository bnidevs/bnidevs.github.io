...
document.getElementById("darkmodecheckbox").addEventListener("change", (e) => {
        ...
        [...document.getElementsByTagName("input")].map(e => {e.classList.toggle('darkmode')});
        [...document.getElementsByTagName("button")].map(e => {e.classList.toggle('darkmode')});

        document.cookie = JSON.stringify({dark: document.getElementById("darkmodecheckbox").checked});
    });